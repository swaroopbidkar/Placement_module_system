...
app.post('/fetch-report', async (req, res) => {
    try {
        ...
        const students = await Student.find({
            department,
            academicyear: { $gte: from_academicyear, $lte: to_academicyear },
            company: { $ne: null, $exists: true } // Filter students who are placed (company field is not null)
        });
        ...
        // Set the headers for the Excel sheet (Updated to replace year with academicyear and changed column order)
        worksheet.columns = [
            { header: 'Student Name', key: 'name', width: 25 },
            { header: 'Phone Number', key: 'phoneNumber', width: 20 },
            { header: 'Email', key: 'email', width: 30 },
            { header: 'Department', key: 'department', width: 20 },
            { header: 'Company Name', key: 'companyName', width: 25 },  // New column for companyName
            { header: 'Academic Year', key: 'academicyear', width: 15 }  // Added academicyear field
        ];
        ...
        const headerRow = worksheet.addRow([
            ...
            'Department', 
            'Company Name',
            'Academic Year'  // Updated header order
        ]);
        ...
        // Add the students data to the Excel sheet (including companyName and academicyear)
        students.forEach(student => {
            if (student.company) { // Ensure the student has a company (i.e., has been placed)
                const studentRow = worksheet.addRow({
                    name: student.name || 'N/A',
                    phoneNumber: student.phoneNumber || 'N/A',
                    email: student.email || 'N/A',
                    department: student.department || 'N/A',
                    companyName: student.company || 'N/A',  // Access companyName directly from the student
                    academicyear: student.academicyear || 'N/A', // Added academicyear
                });
